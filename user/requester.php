...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_request'])) {
    ...
    if ($blood_groups_id && $hospital !== '' && $required_date !== '') {
...
if ($isLoggedIn && $userId > 0) {
    ...
    if ($result && $result->num_rows > 0) {
        $userRequests = $result->fetch_all(MYSQLI_ASSOC);
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Requester Dashboard – BloodLife</title>
  <script>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center h-16">
        ...
        <div class="hidden md:flex items-center space-x-8">
          ...
          <a href="requester.php" class="text-red-600 font-bold transition" data-i18n="my_requests">My Requests</a>
<button type="button" class="theme-toggle-btn relative w-10 h-10 rounded-lg border-2 border-gray-200 bg-gray-50 flex items-center justify-center cursor-pointer hover:border-red-400 transition" aria-label="Toggle theme" onclick="toggleTheme()"><span class="theme-icon-sun">☀️</span><span class="theme-icon-moon" style="display:none">🌙</span></button>
  ...
  <section class="bg-gradient-to-r from-red-600 to-red-800 text-white py-14">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center animate-fade-up">
      <div class="inline-block bg-white/20 px-4 py-2 rounded-full text-sm font-semibold mb-4">📋 My Requests</div>
      <h1 class="text-5xl font-bold mb-3">Requester Dashboard</h1>
      <p class="text-xl opacity-90 max-w-xl mx-auto">Submit and track your blood requests. Every request is reviewed and matched with available donors.</p>
    </div>
  </section>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <?php if ($message): ?>
        <?php if ($messageType === 'error'): ?>
          <div class="rounded-2xl border px-6 py-4 text-sm font-semibold border-red-200 bg-red-50 text-red-700 animate-fade-up">
        <?php else: ?>
          <div class="rounded-2xl border px-6 py-4 text-sm font-semibold border-green-200 bg-green-50 text-green-700 animate-fade-up">
        <?php endif; ?>
            <?= htmlspecialchars($message) ?>
          </div>
      <?php endif; ?>

      <div class="bg-white rounded-2xl shadow p-8 animate-fade-up">
        <div class="flex items-center gap-3 mb-6">
          <div class="w-10 h-10 rounded-xl bg-red-100 flex items-center justify-center text-xl">🩸</div>
          <h2 class="text-xl font-bold text-gray-900">New Blood Request</h2>
        </div>
        <form method="POST" class="grid sm:grid-cols-2 gap-5">
          <div>
            ...
            <select name="blood_groups_id" required class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-red-500 transition bg-white">
              ...
              <?php if ($blood_groups): mysqli_data_seek($blood_groups, 0); while ($bg = $blood_groups->fetch_assoc()): ?>
              <option value="<?= $bg['id'] ?>"><?= htmlspecialchars($bg['blood_gp_name']) ?></option>
              <?php endwhile; endif; ?>
            </select>
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Units Needed <span class="text-red-500">*</span></label>
            <input type="number" name="units" min="1" placeholder="e.g. 2" required
          ...
          <div class="sm:col-span-2">
            ...
            <input type="text" name="hospital" placeholder="e.g. City General Hospital" required
              class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-red-500 transition" />
          </div>
          <div>
          ...
          <div class="sm:col-span-2">
            <button type="submit" name="submit_request"
              class="w-full bg-gradient-to-r from-red-600 to-red-700 text-white px-8 py-4 rounded-xl font-bold hover:shadow-xl hover:from-red-700 hover:to-red-800 transition transform hover:scale-[1.02]">
              Submit Request 🩸
            </button>
          </div>
        </form>
      ...
      <div class="bg-white rounded-2xl shadow p-8 animate-fade-up">
        <div class="flex items-center gap-3 mb-6">
          <div class="w-10 h-10 rounded-xl bg-red-100 flex items-center justify-center text-xl">📋</div>
          <h2 class="text-xl font-bold text-gray-900">My Submitted Requests</h2>
        </div>

        <?php if (!$isLoggedIn): ?>
          <div class="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-6 text-center">
            <p class="text-yellow-700 font-semibold">Please <a href="login.php" class="underline">log in</a> to view your requests.</p>
          </div>
        <?php elseif (count($userRequests) === 0): ?>
          <div class="bg-gray-50 border-2 border-gray-200 rounded-xl p-6 text-center">
            <p class="text-gray-500">You haven't submitted any blood requests yet.</p>
          </div>
        <?php else: ?>
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-gray-50 text-slate-600">
                  ...
                  <th class="p-4 text-left">Blood Type</th>
                  <th class="p-4 text-left">Hospital</th>
                  <th class="p-4 text-left">Units</th>
                  <th class="p-4 text-left">Required Date</th>
              ...
              <tbody>
                ...
                <tr class="border-t border-slate-200 hover:bg-gray-50">
                  ...
                  <td class="p-4"><?= htmlspecialchars($req['blood_gp_name'] ?? '-') ?></td>
                  <td class="p-4"><?= htmlspecialchars($req['hospital'] ?? '-') ?></td>
                  <td class="p-4"><?= htmlspecialchars($req['units'] ?? '-') ?></td>
                  <td class="p-4"><?= htmlspecialchars($req['required_date'] ?? '-') ?></td>
                  <td class="p-4">
                    <?php
                      $s = $req['status'];
                      $badge = match ($s) {
