...
<nav class="bg-white dark:bg-gray-900 shadow-sm sticky top-0 z-30 border-b border-gray-100 dark:border-gray-700 transition-colors duration-300">
                <div class="px-8 py-4 flex justify-between items-center">
                    <div class="flex items-center space-x-4">
                        <div>
                            <h2 class="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-red-700 dark:from-red-400 dark:to-red-500"><?= htmlspecialchars($pageData['title']) ?></h2>
                    ...
                    <div class="flex items-center space-x-4">
                        <!-- Theme Toggle -->
                        <button type="button" class="theme-toggle-btn relative w-10 h-10 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 flex items-center justify-center cursor-pointer hover:border-red-300 dark:hover:border-red-500 hover:bg-red-50 dark:hover:bg-gray-600 transition" aria-label="Toggle theme" onclick="toggleTheme()">
                            <span class="theme-icon-sun"><i class="fas fa-sun text-gray-600 dark:text-gray-300"></i></span>
                            <span class="theme-icon-moon" style="display:none"><i class="fas fa-moon text-gray-600 dark:text-gray-300"></i></span>
                        </button>
                        <!-- Notifications Bell -->
                        <div class="relative">
                            <button onclick="toggleNotifications()" class="relative w-10 h-10 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 flex items-center justify-center hover:bg-red-50 dark:hover:bg-gray-600 hover:border-red-300 dark:hover:border-red-500 transition">
                            ...
                            <!-- Dropdown -->
                            <div id="adminNotifDropdown" class="hidden absolute right-0 mt-3 w-80 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 z-50">
                                <div class="p-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                                    ...
                                    <div>
                                        ...
                                        <?php if ($notifCount > 0): ?>
                                        <a href="?read_notifs=1" class="text-xs text-blue-600 dark:text-blue-400 hover:underline">Mark all as read</a>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <?php if (count($admin_notifs) > 0): ?>
                                    <div class="max-h-80 overflow-y-auto">
                                        <?php foreach ($admin_notifs as $n): 
                                            $is_read = $n['is_read'] == 1;
                                            $bgClass = $is_read ? 'bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700' : 'bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50';
                                        ...
                                        ?>
                                            <div class="block p-4 border-b border-gray-100 dark:border-gray-700 transition <?= $bgClass ?>">
                                                <div class="flex items-start justify-between">
                                                    <p class="text-sm text-gray-900 dark:text-white font-bold flex items-center">
                                                ...
                                                <p class="text-xs text-gray-700 dark:text-gray-300 mt-1 font-medium"><?= htmlspecialchars($n['message']) ?></p>
                                                <div class="mt-2 flex items-center justify-between">
                                                    <p class="text-[11px] text-gray-500 dark:text-gray-400 font-semibold"><?= date('M j, Y · g:i A', strtotime($n['created_at'])) ?></p>
                                                    <a href="<?= htmlspecialchars($read_link) ?>" class="text-[11px] font-bold text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">
                                                        <?= $action_text ?>
                                                    </a>
                                                </div>
                                            </div>
                                ...
                                <?php endif; ?>
                                <div class="p-3 border-t border-gray-100 dark:border-gray-700 text-center bg-gray-50 dark:bg-gray-700 rounded-b-xl">
                                    <a href="notifications.php" class="text-sm font-semibold text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">View All Notifications &rarr;</a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="relative" id="adminMenu">
                            <div class="flex items-center space-x-3 cursor-pointer pl-3 border-l border-gray-200 dark:border-gray-700" onclick="toggleAdminDropdown()">
                                <div class="text-right">
                                    <p class="font-semibold text-sm text-gray-800 dark:text-white"><?= htmlspecialchars($_SESSION['username'] ?? 'Admin') ?></p>
                            ...
                            <div id="adminDropdown" class="hidden absolute right-0 mt-3 w-64 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 z-50">
                                <div class="p-4 border-b border-gray-100 dark:border-gray-700">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <p class="font-semibold text-gray-800 dark:text-white"><?= htmlspecialchars($_SESSION['username'] ?? 'Admin') ?></p>
                                            <p class="text-sm text-gray-400 dark:text-gray-400"><?= htmlspecialchars($_SESSION['user_email'] ?? '') ?></p>
                                        </div>
                                    </div>
                                </div>
                                <div class="p-3">
